...
def nuovo_dato(valori, tempi, prob_base_stazionare=0.8, max_decaduta=0.1, momentum_mult=1, punti_momentum=3, verbose=False):
	...
	prob_staz = max(0.2, prob_base_stazionare - min(0.4, momentum*momentum_mult))
	...
	if np.random.random() < prob_staz:
		...
		delta = np.random.randint(0, valori[-1]*max_decaduta)
