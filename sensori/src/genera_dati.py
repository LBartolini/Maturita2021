...
def nuovo_dato(last_punti, prob_base_stazionare=0.9, max_decaduta=0.05, momentum_mult=1, punti_momentum=3, verbose=False):
	molt_base_momentum = 10**4
	...
	momentum = abs(momentum)*momentum_mult*molt_base_momentum	

	if verbose: print("Momentum ", momentum)

	prob_staz = max(0.2, prob_base_stazionare - min(0.4, momentum))
	...
	if np.random.random() < prob_staz:
		...
		return last_punti[-1][1] # last array con gli ultimi 3 punti (x, y)
	else:
		# trovare nuovo valore
		offset = (min(momentum, 4)*2)/100 # momentum influenza al massimo l'8% del nuovo valore
		delta = np.random.randint(last_punti[-1][1]*offset, last_punti[-1][1]*offset+last_punti[-1][1]*max_decaduta)
		if verbose: print("Min ",last_punti[-1][1]*offset)
		if verbose: print("Max ",last_punti[-1][1]*offset+last_punti[-1][1]*max_decaduta)
